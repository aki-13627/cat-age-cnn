...
model_ft = train_model(model_ft, criterion, optimizer_ft, scheduler, num_epochs=80)
...
def plot_training():
    ...
    filename = f"outputs/logs/training_curve_resnet50_ordinal_regression_{timestamp}.png"
...
def save_model_with_timestamp(model, directory="outputs/checkpoints"):
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    checkpoint_path = os.path.join(directory, f"resnet50_ordinal_regression_{timestamp}.pth")
